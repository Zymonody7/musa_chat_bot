      ...
      <aside className='w-20 md:w-64 bg-surface border-r border-slate-800 flex flex-col shrink-0 transition-all duration-300'>
        ...
        {currentView === 'chat' && (
          <div className='flex-1 overflow-y-auto p-4 space-y-2'>
            <div className='text-xs text-slate-500 mb-2 hidden md:block'>
              对话历史
            </div>
            {sessions.length === 0 ? (
              <div className='text-xs text-slate-500 text-center py-4 hidden md:block'>
                暂无对话记录
              </div>
            ) : (
              sessions.map((session) => (
                ...
                >
                  <div className='text-sm text-white truncate hidden md:block'>
                    {session.title}
                  </div>
